...
这是一套给 **Codex（以及兼容 Codex skill 规范的 AI 编程工具）** 用的技能包。它让 AI 按项目流程做事：先确认需求、再修改代码、检查结果，最后留下任务记录。**1.3.4 补上了自动收尾：正常任务做完后，AI 会接着执行收尾检查，不再只告诉你“可以手动收尾了”。** 安装新版时，全局技能、已登记项目的技能副本和旧收尾规则会一起更新；原来的任务、项目规范和业务代码保留。
...
3. 更新已登记项目里的技能，以及项目 `AGENTS.md`、`.trellis/workflow.md` 和保存的工作流中的收尾规则。
4. 修改说明文件前，把原件备份到用户目录的 `.trellis-skills/backups/`。不改业务源码，不清空任务、规范和日志。
5. 检查 Trellis 主程序是否已安装。遇到失败会明确报错，不能把部分安装当成完成。
...
它会查找这个目录和子项目里的 Trellis 安装，更新已有技能，把旧的“提醒用户手动收尾”接到自动检查流程，并记住项目目录。依赖目录和目录链接会跳过，避免改到别处。以后下载新版再运行 `install.ps1` 即可统一升级。
...
| 6 | 升级技能包 | 运行新版 `install.ps1`，全局技能、已登记项目技能和旧收尾规则一起更新 |
...
| 3 | 技能和旧项目流程一起升级 | 登记一次项目目录。以后运行新版 `install.ps1`，已有技能副本和收尾说明一起更新；原来的任务、规范和业务代码保留 |
...
它会删除 `.agents/skills` 和旧 `.codex/skills` 中属于本包的13个 Trellis 技能，并移除安装器加到全局说明里的入口段落，保留其他用户说明。项目技能、项目 `.trellis` 档案柜和其他技能不会删除。
...
| 2 | 重跑安装脚本 | 在仓库文件夹里再跑一次 `pwsh -File install.ps1`，它会更新全局、已登记项目技能和旧收尾规则 |
| 3 | 开始使用 | 不需要重新 `trellis init`。新开会话加载全局说明；正在工作的会话重新读取项目说明 |
