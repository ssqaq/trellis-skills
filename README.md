...
这是一套给 **Codex（以及兼容 Codex skill 规范的 AI 编程工具）** 用的技能包。装好之后，AI 干活会自动按一套固定流程走：先想清楚再动手、写完代码先做质量检查、发布说明用人话写；当你进入正常收尾流程时，收尾第一步还会再跑一次全量质量检查，检查通过后才存档。还有一键安装脚本，下载后跑一条命令就装好；登记过的项目，今后升级时也会一起更新项目里的技能副本。
...
| 3 | 写完代码和收尾时都做质量检查 | 正常任务流程会跑检查；进入收尾时会再跑一次全量检查，不过关不归档、不算完成 |
...
它会自动完成：把 13 个技能装到你的全局目录、更新已经登记过的项目技能副本、检查 Trellis 主程序装没装，再告诉你下一步做什么。
...
如果项目里本来就有 `.agents\skills` 或 `.codex\skills`，想让以后升级时自动同步，就登记一次项目根目录：

```powershell
pwsh -File sync-local-skills.ps1 -ProjectRoot "D:\你的项目文件夹"
```

它会找到这个项目下面已经存在的 Trellis 技能目录，并把它们更新到当前版本，同时把项目根目录记下来。以后你下载新版本、再运行一次 `install.ps1`，这些已登记项目会自动更新，不需要再手动复制。它只更新 `trellis-*` 技能，不改业务源码、`.trellis` 档案柜和其他技能。
...
| 2 | （AI 改完代码后） | 正常任务流程会触发 trellis-check；进入收尾时，trellis-finish-work 会再自动跑一次全量检查 |
...
| 6 | 升级技能包 | 运行新版 `install.ps1`，全局技能和已登记项目里的 Trellis 技能一起更新 |
...
| 2 | 质量检查在正常任务流程和收尾入口自动触发 | 正常任务流程会检查；你说"收尾吧"或调用 `finish-work` 时，收尾第一步会再做一次全量检查。单纯关闭会话、看到会话变成 idle/completed，不会自己触发检查；绕过任务流程直接改代码，也不会自动补检查 |
| 3 | 项目技能可以自动跟着升级 | 每个项目根目录只需运行一次 `sync-local-skills.ps1 -ProjectRoot <项目目录>` 登记。以后运行新版 `install.ps1` 时，已登记项目里的 Trellis 技能会一起更新 |
...
| 4 | 没建 Trellis 任务，AI 直接改了几行代码，质量检查会跑吗 | 不会。质量检查要靠正常任务流程或 `finish-work` 入口触发。想让检查必跑，就先说"开一个任务"，最后再说"收尾吧" |
...
| 7 | 升级后旧项目要不要重新 init | 不要。升级只覆盖技能（说明书），项目的 `.trellis` 档案柜（任务记录、规范）不受影响 |
| 8 | macOS / Linux 能用吗 | 技能文件夹本身通用，复制到 `~/.agents/skills/` 即可。但 README 里的安装脚本和路径写的是 Windows 版，其他系统需手动安装 |
| 9 | 更新 GitHub 后本地会不会立刻自动变 | 不会后台实时监听 GitHub。下载或拉取新版后运行一次 `install.ps1`，它会自动更新全局技能和已经登记的项目技能 |
