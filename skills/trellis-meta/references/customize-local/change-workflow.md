...
| `in_progress` | check passed | Phase 3.3 (spec update) → 3.4 (authorized commit) → 3.5 (automatically invoke finish-work in this turn) |
| `in_progress` | implementation, specs and authorized commits complete | Phase 3.5 directly; do not wait for a completed status that archive itself writes |
| `completed` | task is still in active tree | Phase 3.5 (run `/trellis:finish-work`; it first runs a fresh full `trellis-check`, then archives) |
...
`.trellis/workflow.md` is the local project workflow, not an immutable template. The user can adapt it to team habits. After editing it, platform entry files may still contain old descriptions, so inspect them too. Closing a chat or seeing an idle/completed status is not a substitute for entering the finish route; the quality gate is guaranteed only when `finish-work` runs.
