...
Phase 3: Finish  -> run the final quality gate, preserve lessons, and wrap up
```
...
| Change the wrap-up flow | Update Phase 3 and `finish-work` related descriptions. Note the current split: Phase 3.4 = AI-driven code commits (batched, user-confirmed), Phase 3.5 = `/finish-work` (fresh full check → archive + record session). `/finish-work` refuses to archive when the quality gate fails or the working tree is dirty. |
...
If only `workflow.md` changes, platform entry files may still contain old language. When the user wants to change "what the AI actually does," also inspect the relevant platform directory. A platform becoming idle or completed is not itself a reliable finish hook; the platform's finish route must invoke `finish-work`.
